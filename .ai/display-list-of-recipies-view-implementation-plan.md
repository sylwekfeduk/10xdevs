...
* **Requirements/Constraints:**
    ...
    * **AI Modification:** AI generates a *new copy* (never overwrites).
...
* **Requirements/Constraints:**
    ...
    * **Query Params:** `page` (default 1), `pageSize` (default 10), `sortBy` (default 'created\_at'), `order` (default 'desc').
...
- **Main elements:** Shadcn/ui `Select` components for:
    * **Sort By:** Options: 'created\_at', 'updated\_at', 'title'.
    * **Sort Order:** Options: 'asc', 'desc'.
    * *Search input (future/not in MVP API).*
...
* **Request:** The `useRecipeLibrary` hook will construct the request with query parameters based on the current `queryState`:
  ...
  ```
    * *Example:* `/api/recipes?page=2&pageSize=10&sortBy=title&order=asc`
