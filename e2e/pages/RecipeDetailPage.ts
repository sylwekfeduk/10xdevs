...
export class RecipeDetailPage {
  ...
  constructor(page: Page) {
    ...
    this.ingredients = page.getByRole('heading', { name: /^ingredients$/i }).locator('..').locator('..');
    this.instructions = page.getByRole('heading', { name: /^instructions$/i }).locator('..').locator('..');
    ...
    this.deleteConfirmButton = page.locator('[role="dialog"] button, .modal button').filter({ hasText: /delete|usuń|confirm|potwierdź/i });
    this.deleteCancelButton = page.locator('[role="dialog"] button, .modal button').filter({ hasText: /cancel|anuluj/i });
    this.aiModifiedBadge = page.locator('[data-testid="ai-badge"], .ai-modified-badge').filter({ hasText: /ai-modified|zmodyfikowany przez ai/i });
  }
